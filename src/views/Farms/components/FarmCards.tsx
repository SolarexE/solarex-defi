...
const FarmCards: React.FC = () => {
  ...
  const srxIndex = farms.findIndex(
    ({ tokenSymbol }) => tokenSymbol === 'SRX',
  )

  const srxPrice =
    srxIndex >= 0 && stakedValue[srxIndex]
      ? stakedValue[srxIndex].tokenPriceInWeth
      : new BigNumber(0)

  const BLOCKS_PER_YEAR = new BigNumber(2336000)
  const SRX_PER_BLOCK = new BigNumber(80)
...
const FarmCard: React.FC<FarmCardProps> = ({ farm }) => {
  ...
  }, [srx, lpTokenAddress, account, setHarvestable])

  const poolActive = true // startTime * 1000 - Date.now() <= 0

  return (
    <StyledCardWrapper>
      {farm.tokenSymbol === 'SRX' && <StyledCardAccent />}
      <Card>
        <CardContent>
          <StyledContent>
